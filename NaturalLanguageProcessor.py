...
class NaturalLanguageProcessor:
	...
	def GetSlots(self, userInput, lastAgenAction):
		...
		#Detect city and product
		for word in userInput.split():
			if word.capitalize() in slotDictionary['city']:
				slots['city'] = word.capitalize()
			elif word.capitalize() in slotDictionary['product']:
				slots['product'] = word.capitalize()

		if 'product' not in slots.keys():
			for word in userInput.split():
				#Choose randomly Grocery or Chinese if user wants Asian
				if word == 'asian':
					slots['product'] = random.choice(['Grocery', 'Electronics'])
				#Mediterranean product for Spanish, Greek and Southern
				elif word in ['spanish', 'greek', 'southern']:		
					slots['product'] = 'Fashion'
